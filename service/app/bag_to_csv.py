...
if len(sys.argv) > 3:
    ...
    listOfBagFiles = [
        f for f in os.listdir(".") if f[-4:] == ".bag"
...
with open(str(configFile)) as f:
    ...
    for topicName in data["topicsToBeAdded"]:
        listOfTopicsToBeAdded.append(topicName)

logging.warning(listOfTopicsToBeAdded)
...
for bagFile in listOfBagFiles:
    ...
    def make_dir(dir):
        logging.warning("Making directory {d}".format(d=dir))
        try:
            os.makedirs(dir)
        except OSError:
    ...
    folder = "./"
    make_dir(os.path.join(folder, "csvs"))
    # get list of topics from the bag
    listOfTopics = []
    for topic, msg, t in bagContents:
        if topic in listOfTopicsToBeAdded:
    ...
    for topicName in listOfTopics:
        ...
        topic_dir = os.path.join(
            folder,
            "csvs",
            "topic=" + topicName.strip("/"),
        )
        make_dir(topic_dir)
        filename = os.path.join(topic_dir, topicName.strip("/") + ".csv")
        print ("Extracting the topic - " + topicName + " to " + filename)
        logging.warning("Extracting the topic - " + topicName + " to " + filename)
